...
try:
    from .timezone_aware_analyzer import TimezoneAwareAnalyzer, TimezoneAwareLag
    TIMEZONE_AWARE_AVAILABLE = True
except ImportError:
    TIMEZONE_AWARE_AVAILABLE = False
    logger.debug("TimezoneAwareAnalyzer niedostępny - używam podstawowej analizy")
...
class LagResult:
    ...
    lag_hours: float          # Lag przeliczony na godziny
    
    def __repr__(self):
        if self.direction == PropagationDirection.LEADS:
...
class SentimentPropagationAnalyzer:
    ...
    def __init__(
        ...
        Args:
            ...
            use_timezone_aware: Czy używać analizy uwzględniającej strefy czasowe
        """
        self.time_resolution = time_resolution_hours
        self.max_lag = int(max_lag_hours / time_resolution_hours)
        self.min_correlation = min_correlation
        self.min_samples = min_samples
        self.use_timezone_aware = use_timezone_aware and TIMEZONE_AWARE_AVAILABLE
        
        # Inicjalizuj timezone-aware analyzer jeśli dostępny i włączony
        self.tz_analyzer = None
        if self.use_timezone_aware:
            try:
                self.tz_analyzer = TimezoneAwareAnalyzer()
                logger.info("Timezone-aware analiza włączona")
            except Exception as e:
                logger.warning(f"Nie można zainicjalizować TimezoneAwareAnalyzer: {e}")
                self.use_timezone_aware = False
        
        logger.info(f"SentimentPropagationAnalyzer: max_lag={max_lag_hours}h, min_corr={min_correlation}, tz_aware={self.use_timezone_aware}")
    ...
    def compute_lag_matrix(
        ...
        use_timezone_aware: Optional[bool] = None
        ...
        Args:
            ...
            use_timezone_aware: Czy używać analizy timezone-aware (domyślnie: self.use_timezone_aware)
        ...
        use_tz = use_timezone_aware if use_timezone_aware is not None else self.use_timezone_aware
        ...
        for i, region_a in enumerate(regions):
            for region_b in regions[i+1:]:
                ...
                if result:
                    # Jeśli timezone-aware jest włączone, skoryguj lag
                    if use_tz and self.tz_analyzer:
                        try:
                            tz_lag = self.tz_analyzer.calculate_timezone_aware_lag(
                                df, region_a, region_b,
                                result.lag_hours, result.correlation
                            )
                            
                            # Użyj skorygowanego lag-u jeśli różni się znacząco
                            if abs(tz_lag.adjusted_lag_hours) < abs(result.lag_hours) * 0.8:
                                # Skorygowany lag jest znacznie mniejszy - użyj go
                                result = LagResult(
                                    region_a=result.region_a,
                                    region_b=result.region_b,
                                    optimal_lag=int(tz_lag.adjusted_lag_hours / self.time_resolution),
                                    correlation=result.correlation,
                                    direction=result.direction,
                                    confidence=result.confidence,
                                    lag_hours=tz_lag.adjusted_lag_hours
                                )
                                logger.debug(f"Skorygowano lag {region_a}→{region_b}: {tz_lag.raw_lag_hours:.1f}h → {tz_lag.adjusted_lag_hours:.1f}h (wakeup: {tz_lag.wakeup_delay_hours:.1f}h)")
                        except Exception as e:
                            logger.warning(f"Błąd timezone-aware analizy dla {region_a}→{region_b}: {e}")
        ...
        logger.info(f"Obliczono {len(lag_matrix)} par lag-ów (timezone-aware: {use_tz})")
        return lag_matrix
