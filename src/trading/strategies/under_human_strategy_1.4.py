"""
UNDERHUMAN STRATEGY v1.4 - OPTIMIZED PROFIT
============================================
Bazuje na sprawdzonej V1.1 (+3.50% ROI) z optymalizacjami:

1. Lepszy risk/reward ratio (ATR TP multiplier 3.5 zamiast 3.0)
2. Wyższe minimalne TP (4.0% zamiast 3.0%)
3. Wyższy próg pewności (8.5 zamiast 8.0) - mniej, ale lepszych transakcji
4. Wcześniejszy trailing stop ($100 zamiast $200) - szybsze zabezpieczenie zysków
5. RSI confirmation - bonus za RSI zgodny z kierunkiem
6. RSI extreme filter - nie handluj gdy RSI przeciwny do kierunku

NIE zawiera (bo pogarszało wyniki w V1.2/V1.3):
- Volatility filtering
- Trend reversal exit
- Equity curve tracking
- Dynamiczny rozmiar pozycji
"""
...
class UnderhumanStrategyV14(BaseStrategy):
    """
    UNDERHUMAN STRATEGY v1.4 - OPTIMIZED PROFIT
    Bazuje na sprawdzonej V1.1 z optymalizacjami zysków.
    """

    name = "UnderhumanStrategyV14"
    description = "Strategia zmiany stanu rynku + anomalie (UNDERHUMAN v1.4 - Optimized)"
    timeframe = "1m"  # Używamy danych minutowych

    def __init__(self, config: dict = None):
        super().__init__(config)

        # === RSI - wskaźnik przegrzania + potwierdzenie momentum ===
        self.rsi_period = self.config.get("rsi_period", 14)
        
        # NOWE V1.4: Progi RSI dla filtrowania ekstremalnych sytuacji
        self.rsi_extreme_high = self.config.get("rsi_extreme_high", 75)
        self.rsi_extreme_low = self.config.get("rsi_extreme_low", 25)

        # === Okna/parametry anomalii (V1.1 compatible) ===
        ...
        # V1.4: WYŻSZY próg pewności (było 8.0 w V1.1, teraz 8.5)
        self.min_confidence_for_trade = self.config.get("min_confidence_for_trade", 8.5)
        ...
        # V1.4: LEPSZY ATR SL/TP - lepszy risk/reward ratio
        self.atr_period = self.config.get("atr_period", 14)
        self.atr_sl_multiplier = self.config.get("atr_sl_multiplier", 2.0)  # SL bez zmian
        self.atr_tp_multiplier = self.config.get("atr_tp_multiplier", 3.5)  # TP: 3.0 → 3.5 (wyższy)
        self.min_sl_percent = self.config.get("min_sl_percent", 2.5)  # SL: 2.0% → 2.5%
        self.min_tp_percent = self.config.get("min_tp_percent", 4.0)  # TP: 3.0% → 4.0% (wyższy)
        
        # Trend filtering (V1.1 compatible)
        ...
        # V1.4: WCZEŚNIEJSZY trailing stop ($200 → $100)
        self.trailing_stop_activation_pnl = self.config.get("trailing_stop_activation_pnl", 100.0)
        self.trailing_stop_atr_multiplier = self.config.get("trailing_stop_atr_multiplier", 1.5)
        
        # Max drawdown protection (V1.1 compatible)
        ...
        if not self._backtest_mode:
            try:
                ...
                logger.info("DydxCollector zainicjalizowany dla UNDERHUMAN v1.4")
        ...
        logger.info(f"Strategia {self.name} zainicjalizowana (UNDERHUMAN v1.4 - Optimized Profit).")
    ...
    def _calculate_sl_tp_atr(self, entry_price: float, side: str, atr_value: float) -> Dict[str, float]:
        """
        V1.4: Oblicza SL/TP używając ATR z lepszym risk/reward ratio.
        """
        if side == "long":
    ...
    def analyze(self, df: pd.DataFrame, symbol: str = "BTC-USD", market_ctx: Optional[Dict[str, Any]] = None) -> Optional[TradingSignal]:
        ...
        # Filtrowanie trendu - nie handluj przeciwko silnym trendom (V1.1 compatible)
        action = None
        reason = []

        # SHORT: OVEREXTENDED/EXHAUSTION
        if state in ("OVEREXTENDED", "EXHAUSTION"):
            # Sprawdź czy trend nie jest zbyt silny w górę
            if trend['direction'] == 'up' and trend['strength'] > 0.7:
                ...
                return None
            # V1.4: RSI extreme filter - nie shortuj gdy RSI już niskie
            if current_rsi < self.rsi_extreme_low:
                logger.info(f"[{self.name}] Odrzucono SHORT - RSI już ekstremalnie niskie ({current_rsi:.1f})")
                return None
        ...
        if state in ("PANIC", "CHAOS"):
            # Sprawdź czy trend nie jest zbyt silny w dół
            if trend['direction'] == 'down' and trend['strength'] > 0.7:
                ...
                return None
            # V1.4: RSI extreme filter - nie kupuj gdy RSI już wysokie
            if current_rsi > self.rsi_extreme_high:
                logger.info(f"[{self.name}] Odrzucono LONG - RSI już ekstremalnie wysokie ({current_rsi:.1f})")
                return None
        ...
        # V1.4: MOCNIEJSZY bonus za zgodność z trendem
        ...
        # V1.4: Bonus za RSI momentum confirmation
        if action == SignalType.BUY and current_rsi < 45:
            confidence += 0.4
        elif action == SignalType.SELL and current_rsi > 55:
            confidence += 0.4
    ...
    def _check_exit(
        ...
        # V1.4: WCZEŚNIEJSZY trailing stop ($100 zamiast $200)
