...
class UnderhumanStrategyV12(BaseStrategy):
    """
    UNDERHUMAN STRATEGY v1.2 - Ulepszona wersja z:
    - ATR-based dynamic SL/TP
    - Trend filtering (nie handluj przeciwko silnym trendom)
    - Trailing stop loss
    - Wyższy próg pewności
    - Ochrona przed max drawdown
    - NOWE: Dynamiczny rozmiar pozycji w zależności od confidence
    - NOWE: Filtrowanie volatility (nie handluj przy zbyt wysokiej zmienności)
    - NOWE: Wyjście gdy trend się odwraca
    - NOWE: Ochrona przed serią strat (cooldown po 3 stratach z rzędu)
    - NOWE: Lepsze zarządzanie ryzykiem
    """

    name = "UnderhumanStrategyV12"
    description = "Strategia zmiany stanu rynku + anomalie (UNDERHUMAN v1.2)"
    timeframe = "5min"
    ...
    def __init__(self, config: dict = None):
        ...
        self.min_sl_percent = self.config.get("min_sl_percent", 2.0)
        self.min_tp_percent = self.config.get("min_tp_percent", 3.0)
        ...
        # Trailing stop
        self.trailing_stop_activation_pnl = self.config.get("trailing_stop_activation_pnl", 200.0)
        ...
        # NOWE: Dynamiczny rozmiar pozycji
        self.base_position_size = self.config.get("base_position_size", 15.0)  # Bazowy rozmiar 15%
        self.max_position_size = self.config.get("max_position_size", 20.0)  # Max 20%
        self.min_position_size = self.config.get("min_position_size", 10.0)  # Min 10%
        
        # NOWE: Filtrowanie volatility
        self.max_volatility_percent = self.config.get("max_volatility_percent", 5.0)  # Nie handluj jeśli volatility > 5%
        self.min_volatility_percent = self.config.get("min_volatility_percent", 0.3)  # Nie handluj jeśli volatility < 0.3%
        self.volatility_period = self.config.get("volatility_period", 24)  # Okres do obliczenia volatility
        
        # NOWE: Ochrona przed serią strat
        self.max_consecutive_losses = self.config.get("max_consecutive_losses", 3)  # Po 3 stratach z rzędu - cooldown
        self.loss_cooldown_seconds = self.config.get("loss_cooldown_seconds", 600)  # 10 minut cooldown
        self.consecutive_losses = 0
        self.last_loss_time: Optional[datetime] = None
        
        # NOWE: Trend reversal detection
        self.trend_reversal_threshold = self.config.get("trend_reversal_threshold", 0.3)  # 30% zmiana siły trendu
        ...
        if not self._backtest_mode:
            try:
                ...
                logger.info("DydxCollector zainicjalizowany dla UNDERHUMAN v1.2")
        ...
        logger.info(f"Strategia {self.name} zainicjalizowana (UNDERHUMAN v1.2).")
    ...
    def _calculate_volatility(self, df: pd.DataFrame, period: int = 24) -> float:
        """
        NOWA: Oblicza volatility jako odchylenie standardowe zmian cen.
        ...
        Returns:
            {
                ...
                'ema_fast': float,
                'ema_slow': float,
                'previous_direction': str  # NOWE: poprzedni kierunek trendu
            }
        """
        if len(df) < self.trend_ema_slow:
            return {'direction': 'sideways', 'strength': 0.0, 'ema_fast': 0.0, 'ema_slow': 0.0, 'previous_direction': 'sideways'}
    ...
    def _detect_trend_reversal(self, trend: Dict[str, Any]) -> bool:
        """
        NOWA: Wykrywa odwrócenie trendu.
        
        Returns:
            True jeśli trend się odwrócił
        """
        if trend['previous_direction'] == 'sideways':
            return False
        
        # Jeśli kierunek się zmienił i siła jest znacząca
        if trend['direction'] != trend['previous_direction']:
            if trend['strength'] > self.trend_reversal_threshold:
                return True
    ...
    def _check_consecutive_losses(self) -> bool:
        """
        NOWA: Sprawdza czy przekroczono limit kolejnych strat.
        
        Returns:
            True jeśli jesteśmy w cooldown po serii strat
        """
        if self.last_loss_time is None:
            return False
        
        elapsed = (datetime.now() - self.last_loss_time).total_seconds()
        return elapsed < self.loss_cooldown_seconds

    def _calculate_dynamic_position_size(self, confidence: float) -> float:
        """
        NOWA: Oblicza dynamiczny rozmiar pozycji w zależności od confidence.
        
        Args:
            confidence: Confidence sygnału (1-10)
        
        Returns:
            Rozmiar pozycji w % (10-20%)
        """
        # Confidence 8.0 = base_position_size (15%)
        # Confidence 10.0 = max_position_size (20%)
        # Confidence < 8.0 = min_position_size (10%)
        
        if confidence >= 8.0:
            # Skalowanie liniowe między base a max
            ratio = (confidence - 8.0) / 2.0  # 0.0-1.0 dla confidence 8.0-10.0
            size = self.base_position_size + (self.max_position_size - self.base_position_size) * ratio
        else:
            # Skalowanie liniowe między min a base
            ratio = (confidence - self.min_confidence_for_trade) / (8.0 - self.min_confidence_for_trade)  # 0.0-1.0
            size = self.min_position_size + (self.base_position_size - self.min_position_size) * ratio
    ...
    def analyze(self, df: pd.DataFrame, symbol: str = "BTC-USD", market_ctx: Optional[Dict[str, Any]] = None) -> Optional[TradingSignal]:
        ...
        # NOWE: Sprawdź cooldown po serii strat
        if self._check_consecutive_losses():
            logger.debug(f"[{self.name}] Cooldown po serii strat - pomijam handel")
            return None
        ...
        # NOWE: Oblicz volatility
        volatility = self._calculate_volatility(df, self.volatility_period)
        
        # NOWE: Filtrowanie volatility - nie handluj przy zbyt wysokiej/niskiej zmienności
        if volatility > self.max_volatility_percent:
            logger.debug(f"[{self.name}] Odrzucono - zbyt wysoka volatility: {volatility:.2f}%")
            return None
        if volatility < self.min_volatility_percent:
            logger.debug(f"[{self.name}] Odrzucono - zbyt niska volatility: {volatility:.2f}%")
            return None
        ...
        # Logi tylko do pliku (nie wyświetlaj w konsoli podczas backtestingu)
        logger.debug(
        ...
        # Filtrowanie trendu - nie handluj przeciwko silnym trendom
        action = None
        reason = []
        ...
        # NOWE: Bonus/kara za volatility
        if 0.5 <= volatility <= 2.0:  # Optymalna volatility
            confidence += 0.5
        elif volatility > 4.0:  # Zbyt wysoka volatility
            confidence -= 0.5
        ...
        reason_str = (
            ...
            f"ATR={current_atr:.2f} | volatility={volatility:.2f}% | "
            f"RSI={current_rsi:.1f} | "
            f"ob_imb={None if ob_imb is None else round(ob_imb, 3)}"
        )

        # Logi tylko do pliku (nie wyświetlaj w konsoli podczas backtestingu)
        logger.debug(f"🎯 [{self.name}] ENTRY {action.value.upper()} conf={confidence:.1f} size={position_size:.1f}% {reason_str}")
        ...
        return TradingSignal(
            ...
            size_percent=position_size,  # NOWE: Dynamiczny rozmiar
            reason=reason_str,
            strategy=self.name,
        )
    ...
    def _check_exit(
        ...
        # NOWE: Wyjście gdy trend się odwraca
        if self._detect_trend_reversal(trend):
            self.last_close_time = datetime.now()
            logger.info(f"[{self.name}] Wyjście - odwrócenie trendu: {trend['previous_direction']} -> {trend['direction']}")
            return TradingSignal(
                signal_type=SignalType.CLOSE,
                symbol=symbol,
                confidence=9.0,
                price=current_price,
                reason=f"TREND REVERSAL {trend['previous_direction']}->{trend['direction']} pnl={pnl:.2f}",
                strategy=self.name
            )
        ...
        if pnl <= -self.max_loss_usd:
            # NOWE: Zwiększ licznik kolejnych strat
            self.consecutive_losses += 1
            self.last_loss_time = datetime.now()
            if self.consecutive_losses >= self.max_consecutive_losses:
                logger.warning(f"[{self.name}] {self.consecutive_losses} kolejnych strat - aktywuję cooldown")
    ...
    def should_close_position(
        ...
        """
        NOWA: Sprawdza czy pozycja powinna być zamknięta (dla BacktestEngine).
        Resetuje licznik kolejnych strat przy zysku.
        """
        if current_pnl_percent > 0:
            # Resetuj licznik kolejnych strat przy zysku
            self.consecutive_losses = 0
            self.last_loss_time = None
        
        return False  # BacktestEngine sam zarządza SL/TP
