...
========================================
Testuje strategię UnderhumanStrategyV10 na danych historycznych z pliku CSV.
Działa w trybie ekspresowym - najszybciej jak się da.

Użycie:
    python tests/integration/test_under_human_strategy.py --csv data/backtest_periods/binance/BTCUSDT_2021_1h.csv
"""
...
from src.trading.strategies.under_human_strategy_1_0 import UnderhumanStrategyV10
...
def load_multiple_csv_files(csv_files: List[Path]) -> Tuple[pd.DataFrame, Dict[int, Tuple[datetime, datetime]]]:
    """
    Wczytuje wiele plików CSV i łączy je w jeden ciągły DataFrame.
    
    Returns:
        Tuple[DataFrame, Dict[int, Tuple[datetime, datetime]]]: 
        - Połączony DataFrame
        - Słownik z zakresami dat dla każdego roku {rok: (start, end)}
    """
    all_dfs = []
    year_ranges = {}
    ...
    for csv_file in sorted(csv_files):
        ...
        # Określ rok z nazwy pliku lub z danych
        year = None
    ...
    if not all_dfs:
        logger.error("❌ Nie udało się wczytać żadnych danych")
        return pd.DataFrame(), {}
    
    # Połącz wszystkie DataFrames
    # Jeśli wszystkie mają timestamp jako index, użyj go
    all_have_timestamp_index = all('timestamp' in df.columns or isinstance(df.index, pd.DatetimeIndex) for df in all_dfs)
    
    if all_have_timestamp_index:
        # Upewnij się, że wszystkie mają timestamp jako index
        ...
        # Jeśli nie wszystkie mają timestamp jako index, użyj kolumny timestamp
        combined_df = pd.concat(all_dfs, ignore_index=False)
    ...
    # Dodaj kolumnę timestamp jeśli nie ma (dla kompatybilności z backtesting engine)
    if 'timestamp' not in combined_df.columns:
        combined_df['timestamp'] = combined_df.index
    
    # Dodaj kolumnę year jeśli nie ma
    if 'year' not in combined_df.columns:
        combined_df['year'] = combined_df['timestamp'].dt.year if 'timestamp' in combined_df.columns else combined_df.index.year
    
    logger.success(f"✅ Połączono {len(all_dfs)} plików w jeden ciągły dataset: {len(combined_df)} świec")
    
    min_date = combined_df.index.min() if isinstance(combined_df.index, pd.DatetimeIndex) else combined_df['timestamp'].min()
    max_date = combined_df.index.max() if isinstance(combined_df.index, pd.DatetimeIndex) else combined_df['timestamp'].max()
    logger.info(f"   Całkowity okres: {min_date} → {max_date}")
    
    return combined_df, year_ranges
...
def main():
    parser = argparse.ArgumentParser(
        description="Backtesting UnderhumanStrategyV10 na danych z pliku CSV",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Przykłady:
  # Test na danych z 2021 roku
  python tests/integration/test_under_human_strategy.py --csv data/backtest_periods/binance/BTCUSDT_2021_1h.csv
  ...
  python tests/integration/test_under_human_strategy.py --csv data/backtest_periods/binance/BTCUSDT_2021_1h.csv \\
    ...
    parser.add_argument(
        ...
        type=str,
        action='append',
        help='Ścieżka do pliku CSV z danymi historycznymi (można użyć wielokrotnie)'
    )
    ...
    # Wczytaj dane
    if not args.csv:
        logger.error("❌ Musisz podać przynajmniej jeden plik CSV (--csv)")
        return 1
    
    csv_paths = [Path(csv) for csv in args.csv]
    
    # Jeśli jest wiele plików, połącz je
    if len(csv_paths) > 1:
        logger.info(f"📂 Wczytuję {len(csv_paths)} plików CSV i łączę je w jeden ciągły dataset...")
