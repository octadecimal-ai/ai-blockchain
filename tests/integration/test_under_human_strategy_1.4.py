...
from src.trading.strategies import UnderhumanStrategyV14
...
LOG_FILE = LOG_DIR / "under_human_strategy_1.4.log"


def setup_logging(verbose: bool = False):
    """Konfiguruje logowanie TYLKO do pliku - bez wyświetlania w konsoli."""
...
def print_detailed_results(result: BacktestResult, year_ranges: Dict[int, Tuple[datetime, datetime]] = None):
    ...
    print("\n" + "=" * 100)
    print("📊 SZCZEGÓŁOWE WYNIKI BACKTESTU - UNDERHUMAN STRATEGY v1.4 (Optimized Profit)")
    print("=" * 100)
...
def main():
    parser = argparse.ArgumentParser(
        description="Backtesting UnderhumanStrategyV14 na danych z bazy danych PostgreSQL",
...
Przykłady:
  ...
  # Test z parametrami
    ...
    # Wyświetl tylko informację o logach
    ...
    # V1.4: Konfiguracja z optymalizacjami
    strategy_config = {
        "_backtest_mode": True,
        # Wyższy próg pewności (8.0 → 8.5)
        "min_confidence_for_trade": 8.5,
        # ATR z lepszym risk/reward
        "atr_period": 14,
        "atr_sl_multiplier": 2.0,
        "atr_tp_multiplier": 3.5,  # Wyższy (było 3.0)
        "min_sl_percent": 2.5,  # Lekko wyższy (było 2.0)
        "min_tp_percent": 4.0,  # Wyższy (było 3.0)
        # Trend filtering (bez zmian)
        "trend_ema_fast": 20,
        "trend_ema_slow": 50,
        "trend_strength_threshold": 0.5,
        # Wcześniejszy trailing stop ($200 → $100)
        "trailing_stop_activation_pnl": 100.0,
    ...
    strategy = UnderhumanStrategyV14(config=strategy_config)
