...
# ============================================================================
# Prompt Strategy v1.2 - Aggressive Dynamic Trading (ULEPSZONA)
# ============================================================================
# Ulepszona strategia LLM z:
# - Automatycznym zamykaniem pozycji przy progach PnL
# - Wymuszaniem poprawnych akcji (nie BUY przy otwartym LONG)
# - Śledzeniem dużych transakcji (whale tracking)
# - Agresywniejszymi progami zamykania
#
# KLUCZOWE RÓŻNICE względem v1.1:
# - Auto Take Profit: +2% (zamiast czekania na LLM)
# - Auto Stop Loss: -1.5% (zamiast czekania na LLM)
# - Max Hold: 4h (zamiast nieskończonego czasu)
# - Walidacja akcji LLM (wymuszanie CLOSE zamiast błędnego BUY)
#
# Użycie:
#   ./scripts/run_prompt_strategy_v12.sh
#   ./scripts/run_prompt_strategy_v12.sh --symbols=BTC-USD,ETH-USD
#   ./scripts/run_prompt_strategy_v12.sh --interval=5min
#
# Autor: AI Assistant
# Data: 2025-12-13
# ============================================================================
...
# === Domyślne parametry ===
# Zoptymalizowane dla AGRESYWNEGO dynamicznego tradingu
INTERVAL="15sek"                 # Częstsze sprawdzanie dla szybszych reakcji
BALANCE=10000
TIME_LIMIT="12h"                # Dłuższa sesja
MODE="paper"
MAX_LOSS=500                    # Realistyczny max loss (5% kapitału)
PROMPT_FILE="prompts/trading/aggressive_dynamic_v12.txt"
SYMBOLS="BTC-USD"
ACCOUNT="prompt_v12_aggressive"
LEVERAGE=6.0                    # Dźwignia
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help|-h)
            echo ""
            echo -e "${MAGENTA}🚀 Prompt Strategy v1.2 - AGGRESSIVE Dynamic Trading${NC}"
            echo ""
            echo "Użycie: $0 [OPCJE]"
            echo ""
            echo "Domyślne parametry (zoptymalizowane dla agresywnego tradingu):"
            echo "  --interval=5min"
            echo "  --balance=10000"
            echo "  --time-limit=12h"
            echo "  --max-loss=500        (5% kapitału)"
            echo "  --prompt-file=prompts/trading/aggressive_dynamic_v12.txt"
            echo "  --symbols=BTC-USD"
            echo "  --account=prompt_v12_aggressive"
            echo "  --leverage=3.0"
            echo ""
            echo "Przykłady:"
            echo "  $0"
            ...
            echo ""
            echo -e "${YELLOW}KLUCZOWE RÓŻNICE względem v1.1:${NC}"
            echo "  ✓ Auto Take Profit: +2%"
            echo "  ✓ Auto Stop Loss: -1.5%"
            echo "  ✓ Max Hold Time: 4h"
            echo "  ✓ Wymuszanie CLOSE zamiast błędnego BUY/SELL"
            echo "  ✓ Whale tracking (duże transakcje)"
            echo ""
            exit 0
            ;;
        *)
...
echo ""
echo -e "${MAGENTA}╔══════════════════════════════════════════════════════════════════╗${NC}"
echo -e "${MAGENTA}║${NC}  ${WHITE}🚀 PROMPT STRATEGY v1.2 - AGGRESSIVE DYNAMIC TRADING${NC}           ${MAGENTA}║${NC}"
echo -e "${MAGENTA}╠══════════════════════════════════════════════════════════════════╣${NC}"
echo -e "${MAGENTA}║${NC}  ${GREEN}NOWE w v1.2:${NC}                                                     ${MAGENTA}║${NC}"
echo -e "${MAGENTA}║${NC}  ✓ Auto Take Profit: +2% (nie czeka na LLM)                       ${MAGENTA}║${NC}"
echo -e "${MAGENTA}║${NC}  ✓ Auto Stop Loss: -1.5% (nie czeka na LLM)                       ${MAGENTA}║${NC}"
echo -e "${MAGENTA}║${NC}  ✓ Max Hold Time: 4h (automatyczne zamknięcie)                    ${MAGENTA}║${NC}"
echo -e "${MAGENTA}║${NC}  ✓ Wymuszanie CLOSE zamiast błędnego BUY/SELL                     ${MAGENTA}║${NC}"
echo -e "${MAGENTA}║${NC}  ✓ Whale tracking (śledzenie dużych transakcji)                   ${MAGENTA}║${NC}"
echo -e "${MAGENTA}╚══════════════════════════════════════════════════════════════════╝${NC}"
echo ""

log_info "Parametry uruchomienia:"
echo -e "  ${WHITE}Strategia:${NC}    prompt_strategy_v12 (LLM + Auto Close + Whale Tracking)"
...
echo ""
echo -e "  ${YELLOW}Auto Take Profit:${NC} +2%"
echo -e "  ${YELLOW}Auto Stop Loss:${NC}   -1.5%"
echo -e "  ${YELLOW}Max Hold Time:${NC}    4h"
echo ""
...
# === Uruchomienie ===
echo ""
log_info "Uruchamiam Prompt Strategy v1.2..."
echo ""
