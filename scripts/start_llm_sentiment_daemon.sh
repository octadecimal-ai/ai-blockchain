...
# ============================================================================
# Start LLM Sentiment Daemon
# ============================================================================
# Uruchamia daemon do zbierania i analizy sentymentu używając LLM w tle.
# Daemon działa nawet po wylogowaniu z systemu.
#
# Użycie:
#   ./scripts/start_llm_sentiment_daemon.sh
#   ./scripts/start_llm_sentiment_daemon.sh --interval=300 --countries=US,CN,JP
#   ./scripts/start_llm_sentiment_daemon.sh --stop  # Zatrzymaj daemon
#   ./scripts/start_llm_sentiment_daemon.sh --status  # Sprawdź status
#
# Autor: AI Assistant
# Data: 2025-12-18
# ============================================================================
...
set -eo pipefail
# Uwaga: usunięto 'u' z set -euo pipefail aby uniknąć problemów z pustymi tablicami w zsh
# set -u powoduje błąd "unbound variable" gdy tablica jest pusta
...
# === Pliki ===
PID_FILE="$PROJECT_ROOT/data/llm_sentiment_daemon.pid"
LOG_FILE="$PROJECT_ROOT/logs/llm_sentiment_daemon.log"
DAEMON_SCRIPT="$SCRIPT_DIR/llm_sentiment_daemon.py"
...
start_daemon() {
    ...
    log_info "Uruchamiam LLM Sentiment Daemon..."
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help|-h)
            ...
            echo "  --symbols=SYMBOL1,SYMBOL2   Symbole do analizy (domyślnie: BTC/USDC)"
            echo "  --countries=COUNTRY1,COUNTRY2   Kraje (domyślnie: US,CN,JP,KR,DE,GB,RU,SG)"
            echo "  --interval=SEKUNDY   Interwał aktualizacji (domyślnie: 600 = 10 min)"
            echo "  --model=MODEL   Model LLM (domyślnie: claude-3-5-haiku-20241022 - tańszy)"
            echo "  --verbose, -v        Szczegółowe logowanie"
            echo ""
            echo "Przykłady:"
            echo "  $0"
            echo "  $0 --interval=300 --countries=US,CN,JP"
            echo "  $0 --stop"
            echo "  $0 --status"
            exit 0
            ;;
...
case $ACTION in
    start)
        # Użyj tablicy tylko jeśli ma elementy (dla zsh compatibility)
