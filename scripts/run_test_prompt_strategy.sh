...
#
# Użycie:
#   ./scripts/run_test_prompt_strategy.sh
#   ./scripts/run_test_prompt_strategy.sh --interval=5s
#
# Autor: AI Assistant
# Data: 2025-12-17
# ============================================================================
...
# === Domyślne parametry ===
INTERVAL="10s"                  # Szybki interwał (10 sekund) - 12 iteracji = ~2 minuty
BALANCE=10000
TIME_LIMIT="30min"              # Limit czasu - wystarczy na 12 iteracji
MODE="paper"
MAX_LOSS=1000                   # Łączna max strata dla sesji
SYMBOLS="BTC-USD"
ACCOUNT="test_prompt_bot"
LEVERAGE=10.0
PROMPT_FILE="prompts/trading/test_prompt_strategy.txt"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help|-h)
            echo ""
            echo -e "${MAGENTA}🧪 Test Prompt Strategy${NC}"
            echo ""
            echo "Testowa strategia do weryfikacji działania systemu:"
            echo "  - Iteracja 0: LONG (~100 USD)"
            echo "  - Iteracje 1-10: WAIT"
            echo "  - Iteracja 11: CLOSE (jeśli pozycja otwarta) lub SHORT (jeśli brak pozycji)"
            echo ""
            echo "Użycie:"
            echo "  $0 [opcje]"
            echo ""
            echo "Opcje:"
            echo "  --interval=INTERVAL      Interwał sprawdzania (domyślnie: 10s)"
            echo "  --balance=BALANCE       Początkowy kapitał (domyślnie: 10000)"
            echo "  --time-limit=TIME       Limit czasu (np. 30min, domyślnie: 30min)"
            ...
            echo "  --account=ACCOUNT       Nazwa konta (domyślnie: test_prompt_bot)"
            ...
            echo "Przykłady:"
            echo "  $0"
            echo "  $0 --interval=5s --time-limit=15min"
            echo "  $0 --balance=50000"
            echo ""
            exit 0
            ;;
        *)
...
log_info ""
log_info "Plan testu:"
log_info "  Iteracja 0:  LONG (~100 USD)"
log_info "  Iteracje 1-10: WAIT"
log_info "  Iteracja 11:  CLOSE/SHORT"
log_info "═══════════════════════════════════════════════════════════════"
echo ""

# === Uruchom strategię ===
log_info "🚀 Uruchamiam testową strategię..."
echo ""
